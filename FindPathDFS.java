...
public class FindPathDFS{
    ...
    public static void addEdge(Vertex source, Vertex destination) {
            edges.add(new Edge(source, destination));
        
    }

    public static void printPath(int source) {
        int temp = 0;
        ...
        for(int k=temp;k<edges.size(); k++){
            System.out.println(edges.get(k).source.getName() + " ");
        }
        System.out.print(destination.getName());
    }

    public static void FindPath (Vertex v, Vertex d){
    ...
    public static boolean DFS (Vertex v, Vertex d){
        v.setVisited(i++);
